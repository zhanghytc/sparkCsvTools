...
class CsvWriter(data:DataFrame, trContext: TrContext, config: TagConfigEntry,tbName:String) extends Writer {
...
//    FileUtils.delete(localFilePath)
    var csvPath = hdfsSrcPath + "/" + tbName + ".csv"
    ...
    files.foreach(file => {
      ...
      if(!HDFSUtils.exists(hdfsSrcPath)) {
        ...
        fs.delete(new Path(hdfsSrcPath), true)
      }
